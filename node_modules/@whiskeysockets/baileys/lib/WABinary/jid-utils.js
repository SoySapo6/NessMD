...
// WhatsApp sometimes delivers mentions/participants as *@lid* identifiers. Those are not phone-number JIDs.
// We maintain a best-effort in-memory map populated from group metadata to translate LIDs back to phone-number JIDs.
const __LID_JID_MAP = new Map();
/** normalize any lid/id to a full *@lid* JID string */
const __normalizeLid = (lid) => {
    if (!lid)
        return '';
    if (typeof lid !== 'string')
        return '';
    if (lid.includes('@')) {
        // already a JID-like string
        return lid.endsWith('@lid') ? lid : lid;
    }
    return `${lid}@lid`;
};
/** cache a single lid -> jid mapping */
const cacheLidToJid = (lid, jid) => {
    const lidNorm = __normalizeLid(lid);
    if (!lidNorm || !jid)
        return;
    const decoded = (0, exports.jidDecode)(lidNorm);
    const lidUser = decoded === null || decoded === void 0 ? void 0 : decoded.user;
    const jidNorm = (0, exports.jidNormalizedUser)(jid);
    if (!jidNorm)
        return;
    __LID_JID_MAP.set(lidNorm, jidNorm);
    if (lidUser)
        __LID_JID_MAP.set(lidUser, jidNorm);
};
exports.cacheLidToJid = cacheLidToJid;
/** cache mappings for a group participant list (as returned by extractGroupMetadata) */
const cacheGroupParticipantMappings = (participants) => {
    if (!Array.isArray(participants))
        return;
    for (const p of participants) {
        if (!p)
            continue;
        // in addressing_mode=lid groups, p.id is usually the LID, p.jid is the phone-number JID (phone_number)
        cacheLidToJid(p.lid || p.id, p.jid || p.id);
        cacheLidToJid(p.id, p.jid || p.id);
    }
};
exports.cacheGroupParticipantMappings = cacheGroupParticipantMappings;
/** best-effort: turn *@lid* JIDs into phone-number *@s.whatsapp.net* JIDs (if we have a cached mapping) */
const lidToJid = (jid) => {
    if (!jid)
        return jid;
    if (!(jid === null || jid === void 0 ? void 0 : jid.endsWith('@lid')))
        return jid;
    const lidNorm = __normalizeLid(jid);
    const decoded = (0, exports.jidDecode)(lidNorm);
    const lidUser = decoded === null || decoded === void 0 ? void 0 : decoded.user;
    const mapped = __LID_JID_MAP.get(lidNorm) || (lidUser ? __LID_JID_MAP.get(lidUser) : undefined);
    // fallback: keep server change for compatibility, but this will still not be a real phone-number if unmapped
    return mapped || lidNorm.replace('@lid', '@s.whatsapp.net');
};
