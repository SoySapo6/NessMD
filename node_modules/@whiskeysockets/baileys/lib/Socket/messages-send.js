...
const makeMessagesSocket = (config) => {
    ...
    const peerSessionsCache = config.peerSessionsCache || new node_cache_1.default({
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        // Request both device + LID protocols so we can learn PN<->LID mappings
        ...
        if (result) {
            const list = (_b = result === null || result === void 0 ? void 0 : result.list) !== null && _b !== void 0 ? _b : [];
            // Store PN<->LID mappings when provided by USync
            const lidPairs = list
                .filter((a) => !!(a === null || a === void 0 ? void 0 : a.lid) && !!(a === null || a === void 0 ? void 0 : a.id))
                .map((a) => ({
                lid: (0, WABinary_1.jidNormalizedUser)(a.lid),
                pn: (0, WABinary_1.jidNormalizedUser)(a.id)
            }));
            if (lidPairs.length && ((_c = signalRepository.lidMapping) === null || _c === void 0 ? void 0 : _c.storeLIDPNMappings)) {
                try {
                    await signalRepository.lidMapping.storeLIDPNMappings(lidPairs);
                }
                catch (e) {
                    logger.warn({ e, count: lidPairs.length }, 'failed to store LID mappings from USync');
                }
                // Proactively refresh sessions for newly discovered LIDs to reduce "No sessions"
                try {
                    await assertSessions(lidPairs.map(p => p.lid), true);
                }
                catch (e) {
                    logger.warn({ e }, 'failed to assert sessions for newly mapped LIDs');
                }
            }
            // Extract devices, preferring LID identity when present
            const { user: myUser, device: myDevice } = (0, WABinary_1.jidDecode)(authState.creds.me.id);
            const extracted = [];
            for (const userResult of list) {
                const baseId = (userResult === null || userResult === void 0 ? void 0 : userResult.lid) || (userResult === null || userResult === void 0 ? void 0 : userResult.id);
                if (!baseId)
                    continue;
                const decoded = (0, WABinary_1.jidDecode)(baseId);
                if (!decoded)
                    continue;
                const user = decoded.user;
                const deviceList = userResult === null || userResult === void 0 ? void 0 : userResult.devices?.deviceList;
                if (!Array.isArray(deviceList))
                    continue;
                for (const { id: device, keyIndex } of deviceList) {
                    if ((myUser !== user || myDevice !== device) &&
                        (!ignoreZeroDevices || device !== 0) &&
                        (device === 0 || !!keyIndex)) {
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (force) {
            ...
            for (const jid of uniqueJids) {
                ...
                const cached = peerSessionsCache.get(signalId);
                if (cached === true) {
                    continue;
                }
                // validateSession is a backport from baileys-master; fall back to key existence if absent
                let hasSession = false;
                if (typeof signalRepository.validateSession === 'function') {
                    const res = await signalRepository.validateSession(jid);
                    hasSession = !!(res === null || res === void 0 ? void 0 : res.exists);
                }
                else {
                    const addr = signalRepository.jidToSignalProtocolAddress(jid);
                    const sess = await authState.keys.get('session', [addr]);
                    hasSession = !!sess && !!sess[addr];
                }
        ...
        if (jidsRequiringFetch.length) {
            // When LID addressing is enabled, WA expects keys fetched using LID JIDs.
            // Try to map PN -> LID using the mapping store.
            const lidList = jidsRequiringFetch.filter(j => (0, WABinary_1.isLidUser)(j) || (typeof WABinary_1.isHostedLidUser === 'function' && (0, WABinary_1.isHostedLidUser)(j)));
            const pnList = jidsRequiringFetch.filter(j => (0, WABinary_1.isJidUser)(j) && !(0, WABinary_1.isLidUser)(j));
            let wireJids = [...lidList];
            if (((_a = signalRepository.lidMapping) === null || _a === void 0 ? void 0 : _a.getLIDsForPNs) && pnList.length) {
                try {
                    const mapped = await signalRepository.lidMapping.getLIDsForPNs(pnList);
                    if (mapped && mapped.length) {
                        wireJids.push(...mapped.map(m => m.lid));
                    }
                    else {
                        wireJids.push(...pnList);
                    }
                }
                catch (e) {
                    logger.warn({ e }, 'LID mapping lookup failed; falling back to PN for session fetch');
                    wireJids.push(...pnList);
                }
            }
            else {
            ...
            wireJids = Array.from(new Set(wireJids)).filter(Boolean);
            logger.debug({ count: wireJids.length }, 'fetching sessions');
            const chunkSize = 50;
            for (let i = 0; i < wireJids.length; i += chunkSize) {
                const chunk = wireJids.slice(i, i + chunkSize);
                const result = await query({
                    tag: 'iq',
                    attrs: {
                        xmlns: 'encrypt',
                ...
                for (const i in album) {
                    ...
                    if ('image' in media) {
                        mediaMsg = await (0, Utils_1.generateWAMessage)(jid, {
                            ...
                            userJid,
                            fastMediaProcessing: config.fastMediaProcessing,
