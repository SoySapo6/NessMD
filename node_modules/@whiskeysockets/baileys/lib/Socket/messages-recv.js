...
const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, getMessage, shouldIgnoreJid, enableParallelMessageProcessing, maxParallelMessageThreads, } = config;
    ...
    const messageProcessingMutex = enableParallelMessageProcessing
        ? (0, make_mutex_1.makeKeyedMutex)()
        : { mutex: (_key, task) => processingMutex.mutex(task) };
    const messageProcessingLimit = Math.max(1, maxParallelMessageThreads || 1);
    const messageSemaphore = enableParallelMessageProcessing && messageProcessingLimit > 1
        ? makeSemaphore(messageProcessingLimit)
        : null;
    const runWithMessageSemaphore = async (task) => messageSemaphore
        ? messageSemaphore.run(task)
        : task();
    ...
    const handleMessage = async (node) => {
        var _a, _b, _c;
        return runWithMessageSemaphore(async () => {
            ...
            try {
                const mutexKey = node.attrs.from || node.attrs.participant || node.attrs.id || 'unknown';
                await Promise.all([
                    messageProcessingMutex.mutex(mutexKey, async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if (isAnyHistoryMsg) {
                            ...
                            receiptTasks.push(sendReceipt(jid, undefined, [msg.key.id], 'hist_sync'));
                        }
                        const ackPromise = sendMessageAck(node);
                        // --- LID -> phone-number JID normalization for bot-facing message objects ---
                        if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid)) {
                            try {
                                const needsResolve = (() => {
                                    if ((msg.key.participant && (0, WABinary_1.isLidUser)(msg.key.participant)) ||
                                        (msg.key.remoteJid && (0, WABinary_1.isLidUser)(msg.key.remoteJid))) {
                                        return true;
                                    }
                                    const scan = (obj) => {
                                        ...
                                        if (ci) {
                                            if (ci.participant && (0, WABinary_1.isLidUser)(ci.participant))
                                                return true;
                                            const mj = ci.mentionedJid || ci.mentionedJidList;
                                            if (Array.isArray(mj) && mj.some(x => x && (0, WABinary_1.isLidUser)(x)))
                                                return true;
                                        }
                                        return false;
                                    };
                                    const mm = msg.message || {};
                                    return scan(mm.extendedTextMessage) || scan(mm.imageMessage) || scan(mm.videoMessage) ||
                                        scan(mm.documentMessage) || scan(mm.audioMessage) || scan(mm.stickerMessage) ||
                                        scan(mm.buttonsResponseMessage) || scan(mm.listResponseMessage) ||
                                        scan(mm.templateButtonReplyMessage) || scan(mm.buttonsMessage) || scan(mm.templateMessage) ||
                                        scan(mm.interactiveResponseMessage);
                                })();
                                if (needsResolve) {
                                    const metadata = await groupMetadata(msg.key.remoteJid);
                                    if (typeof WABinary_1.cacheGroupParticipantMappings === 'function') {
                                        WABinary_1.cacheGroupParticipantMappings(metadata.participants);
                                    }
                                    const mapOne = (j) => {
                                        if (!j)
                                            return j;
                                        if (!(0, WABinary_1.isLidUser)(j))
                                            return j;
                                        const fp = metadata.participants.find(p => p.lid === j || p.id === j || p.jid === j);
                                        return (fp === null || fp === void 0 ? void 0 : fp.jid) || (0, WABinary_1.lidToJid)(j);
                                    };
                                    // fix key jids (prefer *_pn attrs when WA provides them)
                                    if (msg.key.participant && (0, WABinary_1.isLidUser)(msg.key.participant)) {
                                        msg.key.participant = node.attrs.participant_pn || mapOne(msg.key.participant) || msg.key.participant;
                                    }
                                    if (msg.key.remoteJid && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                                        msg.key.remoteJid = node.attrs.sender_pn || node.attrs.peer_recipient_pn || mapOne(msg.key.remoteJid) || msg.key.remoteJid;
                                    }
                                    const fixContextInfo = (ci) => {
                                    ...
                                    const walk = (obj, depth = 0) => {
                                        if (!obj || typeof obj !== 'object' || depth > 6)
                                            return;
                                        if (obj.contextInfo)
                                            fixContextInfo(obj.contextInfo);
                                ...
                                // ignore resolution errors; message will still be processed
                            }
                        }
                        (0, Utils_1.cleanMessage)(msg, authState.creds.me.id);
                        await upsertMessage(msg, node.attrs.offline ? 'append' : 'notify');
                        await Promise.all([ackPromise, ...receiptTasks]);
                        return;
                    }
                    (0, Utils_1.cleanMessage)(msg, authState.creds.me.id);
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
