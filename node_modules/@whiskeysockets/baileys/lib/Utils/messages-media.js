...
exports.getStatusCodeForMediaRetry = exports.decryptMediaRetryData = exports.decodeMediaRetryNode = exports.encryptMediaRetryRequest = exports.getWAUploadToServer = exports.getRawMediaUploadData = exports.downloadEncryptedContent = exports.downloadContentFromMessage = exports.getUrlFromDirectPath = exports.encryptedStream = exports.prepareStream = exports.getHttpStream = exports.getStream = exports.toBuffer = exports.toReadable = exports.getVideoDuration = exports.convertAudioToOpus = exports.mediaMessageSHA256B64 = exports.generateProfilePicture = exports.encodeBase64EncodedStringForUpload = exports.extractImageThumb = exports.extractVideoThumb = exports.hkdfInfoKey = void 0;
...
const convertAudioToOpus = async (media, logger) => {
    ...
    const inputPath = (0, path_1.join)(tmpDir, `baileys-ptt-input-${(0, generics_1.generateMessageIDV2)()}`);
    const outputPath = (0, path_1.join)(tmpDir, `baileys-ptt-output-${(0, generics_1.generateMessageIDV2)()}.ogg`);
    let inputIsTemp = false;
    try {
        if (Buffer.isBuffer(media)) {
            await fs_1.promises.writeFile(inputPath, media);
            inputIsTemp = true;
        }
        else if (typeof media === 'string') {
            await fs_1.promises.copyFile(media, inputPath);
            inputIsTemp = true;
        }
        else if ('url' in media) {
            if (media.url instanceof URL && media.url.protocol.startsWith('http')) {
                const stream = await (0, exports.getHttpStream)(media.url.toString());
                const buffer = await (0, exports.toBuffer)(stream);
                await fs_1.promises.writeFile(inputPath, buffer);
                inputIsTemp = true;
            }
            else {
                await fs_1.promises.copyFile(media.url.toString(), inputPath);
                inputIsTemp = true;
            }
        }
        else {
            const buffer = await (0, exports.toBuffer)(media.stream);
            await fs_1.promises.writeFile(inputPath, buffer);
            inputIsTemp = true;
        }
        logger === null || logger === void 0 ? void 0 : logger.debug('converting audio to opus for ptt');
        ...
        if (code !== 0) {
            throw new boom_1.Boom('ffmpeg failed to convert audio to opus', { statusCode: 500 });
        }
        const converted = await fs_1.promises.readFile(outputPath);
        return converted;
    }
    finally {
        try {
...
const getHttpStream = async (url, options = {}) => {
    const response = await fetch(url.toString(), {
        dispatcher: options.dispatcher || options.agent || options.httpsAgent,
        method: 'GET',
    ...
    // Node18+ Readable.fromWeb exists
    const body = response.body;
    if (!body) {
        throw new boom_1.Boom(`Empty response body from ${url}`, { statusCode: 500, data: { url } });
    }
    // @ts-ignore
    return body instanceof stream_1.Readable ? body : stream_1.Readable.fromWeb(body);
};
...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
    ...
    try {
        ...
        originalFileStream === null || originalFileStream === void 0 ? void 0 : originalFileStream.end();
        stream.destroy();
        ...
        originalFileStream === null || originalFileStream === void 0 ? void 0 : originalFileStream.destroy();
        try {
            aes.destroy();
        }
        catch (_a) { }
...
const uploadMedia = async (params, logger) => {
    if (isNodeRuntime()) {
        logger === null || logger === void 0 ? void 0 : logger.debug('Using Node.js http(s) module for upload');
        return uploadWithNodeHttp(params);
    }
    logger === null || logger === void 0 ? void 0 : logger.debug('Using Fetch API for upload');
    return uploadWithFetch(params);
};
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (fileOrStream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        var _a, _b;
        let tmpPath;
        let filePath;
        // Support legacy callers that pass a stream/buffer by writing to a temp file.
        if (typeof fileOrStream === 'string') {
            filePath = fileOrStream;
        }
        else {
            tmpPath = (0, path_1.join)((0, os_1.tmpdir)(), `wa-upload-${mediaType}-${(0, generics_1.generateMessageIDV2)()}`);
            const ws = (0, fs_1.createWriteStream)(tmpPath);
            try {
                if (Buffer.isBuffer(fileOrStream)) {
                    ws.end(fileOrStream);
                    await (0, events_1.once)(ws, 'finish');
                }
                else {
                    for await (const chunk of fileOrStream) {
                        if (!ws.write(chunk)) {
                            await (0, events_1.once)(ws, 'drain');
                        }
                    }
                    ws.end();
                    await (0, events_1.once)(ws, 'finish');
                    fileOrStream.destroy();
                }
            }
            catch (e) {
                ws.destroy();
        ...
        try {
            ...
            const customHeaders = (() => {
                var _a;
                const hdrs = (_a = options === null || options === void 0 ? void 0 : options.headers) !== null && _a !== void 0 ? _a : undefined;
                if (!hdrs)
                    return {};
