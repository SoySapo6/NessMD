...
exports.assertMediaContent = exports.downloadMediaMessage = exports.aggregateMessageKeysNotFromMe = exports.updateMessageWithPollUpdate = exports.updateMessageWithReaction = exports.updateMessageWithReceipt = exports.getDevice = exports.extractMessageContent = exports.normalizeMessageContent = exports.getContentType = exports.generateWAMessage = exports.generateWAMessageFromContent = exports.generateWAMessageContent = exports.generateForwardMessageContent = exports.prepareDisappearingMessageSettingContent = exports.prepareWAMessageMedia = exports.prepareFloatingMessage = exports.generateLinkPreviewIfRequired = exports.extractUrlFromText = void 0;
...
const prepareFloatingMessage = ({ text, title, body, sourceUrl, thumbnail, thumbnailUrl, mediaType = 1, renderLargerThumbnail = true, showAdAttribution = false, }) => {
    ...
    if (thumbnailUrl) {
        externalAdReply.thumbnailUrl = thumbnailUrl;
    }
    return {
        text,
        contextInfo: {
...
exports.prepareFloatingMessage = prepareFloatingMessage;
...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const isVoiceNote = mediaType === 'audio' && uploadData.ptt === true;
    const isOpusMime = typeof uploadData.mimetype === 'string' &&
        (uploadData.mimetype.includes('opus') || uploadData.mimetype === 'audio/ogg');
    if (isVoiceNote && !isOpusMime) {
        uploadData.media = await (0, messages_media_1.convertAudioToOpus)(uploadData.media, logger);
    ...
    const fastMediaProcessing = options.fastMediaProcessing === true;
const requiresAudioDurationComputation = mediaType === 'audio' &&
    typeof uploadData.seconds === 'undefined' &&
    (!fastMediaProcessing || uploadData.ptt === true);
const requiresVideoDurationComputation = mediaType === 'video' && typeof uploadData.seconds === 'undefined';
const requiresThumbnailComputation = !fastMediaProcessing && (mediaType === 'image' || mediaType === 'video') &&
    (typeof uploadData['jpegThumbnail'] === 'undefined');
const requiresWaveformProcessing = mediaType === 'audio' && uploadData.ptt === true;
const requiresAudioBackground = !fastMediaProcessing && options.backgroundColor && mediaType === 'audio' && uploadData.ptt === true;
const requiresOriginalForSomeProcessing = requiresAudioDurationComputation || requiresVideoDurationComputation || requiresThumbnailComputation || requiresWaveformProcessing;
...
// Newsletter uploads (raw) keep legacy behavior but are compatible with faster uploader.
if (options.newsletter) {
    const { mediaKey: mk, encWriteStream, bodyPath, fileEncSha256: fe, fileSha256: fs, fileLength: fl, didSaveToTmpPath } = await (0, messages_media_1.prepareStream)(uploadData.media, options.mediaTypeOverride || mediaType, {
        logger,
    ...
    const up = await options.upload(encWriteStream, {
        fileEncSha256B64,
        mediaType,
    ...
    const { mediaKey: mk, encFilePath, originalFilePath, fileEncSha256: fe, fileSha256: fs, fileLength: fl } = await (0, messages_media_1.encryptedStream)(uploadData.media, options.mediaTypeOverride || mediaType, {
        logger,
    ...
    const [up] = await Promise.all([
        ...
        (async () => {
            try {
                const bodyPath = originalFilePath;
                if (requiresThumbnailComputation && bodyPath) {
                ...
                if (requiresAudioDurationComputation && bodyPath) {
                    uploadData.seconds = await (0, messages_media_1.getAudioDuration)(bodyPath);
                    logger === null || logger === void 0 ? void 0 : logger.debug('computed audio duration');
                }
                if (requiresVideoDurationComputation && bodyPath) {
                    uploadData.seconds = await (0, messages_media_1.getVideoDuration)(bodyPath);
                    logger === null || logger === void 0 ? void 0 : logger.debug('computed video duration');
                }
                if (requiresWaveformProcessing && bodyPath) {
...
const toJid = (id) => {
    if (!id)
        return '';
    if (id.endsWith('@lid'))
        return (0, WABinary_1.lidToJid)(id);
    if (id.includes('@'))
        return id;
...
const getSenderLid = (message) => {
    ...
    // return a non-@lid identifier for compatibility; best-effort resolve to phone-number JID if cached
    const resolved = (0, WABinary_1.lidToJid)(lid);
    return { jid: sender, lid: resolved };
};
